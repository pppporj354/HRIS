...
export class DashboardQuickActions {
    ...
    async updatePendingApprovals() {
        try {
            ...
            if (data.success) {
                ...
                const leaveCountElement = document.getElementById('pendingLeaveCount');
                if (leaveCountElement) {
                    leaveCountElement.textContent = data.data.leave_requests.count;
                ...
                const payrollCountElement = document.getElementById('pendingPayrollCount');
                if (payrollCountElement) {
                    payrollCountElement.textContent = data.data.payroll_pending.count;
                }
            }
        } catch (error) {
    ...
    async updateSystemStatus() {
        try {
            ...
            if (data.success && data.data.length > 0) {
                const avgAttendanceRate = data.data.reduce((sum, emp) => sum + emp.attendance_rate, 0) / data.data.length;
                
                const attendanceElement = document.getElementById('avgAttendanceRate');
                if (attendanceElement) {
                    attendanceElement.textContent = Math.round(avgAttendanceRate) + '%';
                    
                    // Update color based on performance
                    attendanceElement.classList.remove('text-success', 'text-warning', 'text-danger');
            ...
            const leaveResponse = await fetch('/api/dashboard/leave-statistics');
            const leaveData = await leaveResponse.json();
            
            if (leaveData.success) {
                const currentMonth = new Date().getMonth() + 1;
                const monthlyData = leaveData.data.monthly_trends.find(item => item.month == currentMonth);
                
                if (monthlyData) {
                    const monthlyLeaveElement = document.getElementById('monthlyLeaveRequests');
                    if (monthlyLeaveElement) {
                        monthlyLeaveElement.textContent = monthlyData.total_requests;
                    }
                }
            }

            // Update monthly payroll amount
            const payrollResponse = await fetch('/api/dashboard/monthly-payroll');
            const payrollData = await payrollResponse.json();
            
            if (payrollData.success) {
                const currentMonth = new Date().getMonth() + 1;
                const monthlyPayroll = payrollData.data.find(item => item.month == currentMonth);
                
                if (monthlyPayroll) {
                    const payrollElement = document.getElementById('monthlyPayrollAmount');
                    if (payrollElement) {
    ...
    async exportReport(type) {
        const loadingToast = this.showLoadingToast(`Mengexport laporan ${type}...`);
        ...
        try {
            ...
            switch (type) {
                case 'attendance':
                    url = '/export/absensi';
                    filename = `laporan-absensi-${new Date().toISOString().split('T')[0]}.xlsx`;
                    break;
                case 'payroll':
                    url = '/export/penggajian';
                    filename = `laporan-penggajian-${new Date().toISOString().split('T')[0]}.xlsx`;
                    break;
                case 'employees':
                    url = '/export/karyawan';
                    filename = `data-karyawan-${new Date().toISOString().split('T')[0]}.xlsx`;
                    break;
                case 'leave':
                    url = '/export/cuti';
                    filename = `laporan-cuti-${new Date().toISOString().split('T')[0]}.xlsx`;
                    break;
                default:
            ...
            this.hideLoadingToast(loadingToast);
            this.showErrorToast(`Gagal mengexport laporan ${type}. Silakan coba lagi.`);
